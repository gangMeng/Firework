...
void World::SetPara(int wind,int ambient,int stardensity,int moonglow,int moon,int clouds,int earth)
{
 //初始化参数
   dWind = wind;
   dAmbient = ambient;
   dStardensity = stardensity;
   dMoonglow = moonglow;
   dMoon = moon;
   dClouds = clouds;
   dEarth = earth;
...
void World::InitWorld(int wind,int ambient,int stardensity,int moonglow,int moon,int cloud,int earth)
{
   int i, j;
   float x, y, z;	
   ...
   gluBuild2DMipmaps(GL_TEXTURE_2D, 2, CLOUDTEXSIZE, CLOUDTEXSIZE, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, cloudmap);
	...
	if(dStardensity)
	{
	  ...
	  int u, v;
	  unsigned int rgb[3];
	  for(i=0; i<(dStardensity*100); i++)//随机分布星空
	  {
		...
		case 14:  // X-large
			...
			starmap[u+1][v+1][0]=starmap[u+1][v-1][0]=starmap[u-1][v+1][0]=starmap[u-1][v-1][0]=rgb[0];
			starmap[u+1][v+1][1]=starmap[u+1][v-1][1]=starmap[u-1][v+1][1]=starmap[u-1][v-1][1]=rgb[1];
			starmap[u+1][v+1][2]=starmap[u+1][v-1][2]=starmap[u-1][v+1][2]=starmap[u-1][v-1][2]=rgb[2];
		 }
	  }
	...
	if(dMoonglow)
	{
	  ...
	  for(i=0; i<MOONGLOWTEXSIZE; i++)
	  {
		for(j=0; j<MOONGLOWTEXSIZE; j++)
		{
		  ...
		  if(temp3 < 0.0f) temp3 = 0.0f;
		  temp3 = temp3 * temp3 * temp3 * temp3;
				
		  moonglowmap[i][j][0] = (unsigned char)(125.0f * (temp1 * 0.45f + temp2 * 0.4f + temp3 * 0.6f));
		  moonglowmap[i][j][1] = (unsigned char)(125.0f * (temp1 * 0.45f + temp2 * 0.54f + temp3 * 0.45f));
		  moonglowmap[i][j][2] = (unsigned char)(125.0f * (temp1 * 0.6f + temp2 * 0.4f + temp3 * 0.45f));
	...
	if(doSunset)
	{
	  ...
	  unsigned char rgb[3];
	  float temp;
	  if(rsRandi(3)) rgb[0] = 60 + rsRandi(42);
	  else rgb[0] = rsRandi(102);
	  rgb[1] = rsRandi(rgb[0]);
	  rgb[2] = 0;
	  if(rgb[1] < 50) rgb[2] = 100 - rsRandi(rgb[0]);
	...
	if(dStardensity)
	{
	 for(j=0; j<STARMESH/2; j++)
	 {
	   ...
	   for(i=0; i<=STARMESH; i++)
	   {
		 ...
		 // brightness
		 if(stars[i][j][1] < 1501.0f) stars[i][j][5] = 0.0f;
		 else stars[i][j][5] = 1.0f;
		}
	 }
	...
	if(dClouds)
	{
	  ...
	  for(j=0; j<=CLOUDMESH; j++)
	  {
		for(i=0; i<=CLOUDMESH; i++)
		{
		  ...
		  clouds[i][j][5] = (clouds[i][j][1] - 1000.0f) * 0.00001f * float(dAmbient*0.5);  // brightness
		  if(clouds[i][j][5] < 0.0f) clouds[i][j][5] = 0.0f;
		 }
	   }
	}
...
void World::update(float frameTime)
{
  const float recipHalfCloud = 1.0f / float(CLOUDMESH / 6);
	...
	cloudShift += 0.0015f * float(dWind) * frameTime;
	while(cloudShift > 1.0f) cloudShift -= 1.0f;
...
void World::draw()
{
	...
	if(dMoonglow && dMoon)
	{
	  ...
	  //glCallList(m_Scene->m_Flare.flarelist[0]);  // spot
	  glPopMatrix();
	}
